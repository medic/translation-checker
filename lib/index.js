...
async function checkTranslations(
  ...
  if (checkPlaceholders) {
    if (fileNames.indexOf(ENGLISH_TRANSLATIONS) >= 0) {
      enTranslations = await parseProperties(path.join(dir, ENGLISH_TRANSLATIONS));
    }
    if (fileNames.indexOf(EXTRA_PLACEHOLDERS) >= 0) {
      exTranslations = await parseProperties(path.join(dir, EXTRA_PLACEHOLDERS));
    }
  ...
  for (const fileName of fileNames) {
    ...
    if (fileName !== EXTRA_PLACEHOLDERS) {
      errors.push(
