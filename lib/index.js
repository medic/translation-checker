 ...
 * @param {{checkPlaceholders: boolean}} options - optional arguments to perform the translations check
...
async function checkTranslations(
  dir,
  options = {checkPlaceholders: true}
) {
  const errors = [];
  ...
  for (const fileName of fileNames) {
    ...
    if (fileName !== EX_FILE) {
      errors.push(...checkFileTranslations(translations, fileName, templatePlaceholders));
    }
  }
  if (errors.length) {
...
function checkFileTranslations(translations, fileName, templatePlaceholders) {
  const errors = [];
...
function extractPlaceholdersFromTranslations(translations, extraPlaceholders = {}) {
  ...
  for (const [msgKey, msgSrc] of Object.entries(translations)) {
    ...
    if (typeof msgSrc === 'string') {
      msgPlaceholders = msgSrc.match(MUSTACHE_MATCHER) || [];
    }
    if (typeof extraPlaceholders[msgKey] === 'string') {
      msgPlaceholders =
        msgPlaceholders.concat(extraPlaceholders[msgKey].match(MUSTACHE_MATCHER)) || [];
    }
