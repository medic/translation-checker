 ...
 * @param {string[]} languages - the lowercase ISO 639-1 code languages to be checked. Defaults to all valid files found in `dir`.
 ...
 * @param {boolean} [checkMissing=false] - whether to ensure all keys in en file also exist in other languages
 *
...
async function checkTranslations(
  ...
  const knownKeys = getKnownKeys(englishTranslations, checkMissing);
    ...
    if (translations) {
      errors.push(
        ...checkFileTranslations(
          translations, fileName, templatePlaceholders, knownKeys,
...
function getKnownKeys(defaultTranslations, checkMissing) {
  if (!checkMissing) {
    return [];
  }
...
function checkFileTranslations(
  ...
  const foundKeys = Object.keys(translations);
  for (const known of knownKeys) {
    if (!foundKeys.includes(known)) {
      errors.push(parserMissedKey(lang, known));
    }
  }
  return errors;
}
...
function parserMissedKey(lang, msgKey) {
  return {
    lang: lang,
    error: 'missing-key',
    key: msgKey,
    message: `Missing key found for key '${msgKey}' in '${lang}' translation`
  };
}
